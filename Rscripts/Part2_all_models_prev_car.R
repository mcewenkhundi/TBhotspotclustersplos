# Project: TBhotspots
# Task: run all the model for prevalence with the car term
...
paste0(allModelsList[[30]], " + ", "offset(log(tent_cxr_total))")
...
for (i in seq_along(allModelsList[1:32])) {
  ...
  if (allModelsList[1:32][[i]] == "n_prev_tbcases ~ 1") {
    model <- model_function(
      model = paste0(allModelsList[1:32][[i]], " + ", "offset(log(tent_cxr_total))"),
  ...
  print(paste0("prev_model_car_", i))
  saveRDS(object = model, file = here::here("data", paste0("prev_model_car_", i, ".rds")))
  rm(model)
  gc()
}
